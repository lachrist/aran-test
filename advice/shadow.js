...
module.exports = (aran, join) => {
  ...
  const produce = (value, serial) => values[values.length] = value;

  const consume = (value, serial) => {
    check("Consume", values[values.length-1], value, serial);
    values.length--;
    return value;
  };
  ...
  const read = (binding, identifier, value, serial) => {
    check("Read", binding[identifier], value, serial);
    if (binding === global && Reflect_apply(WeakMap_prototype_has, substitutes, [value]))
      return produce(Reflect_apply(WeakMap_prototype_get, substitutes, [value]));
    return produce(value, serial);
  };

  const enter = (type, serial, binding) => {
  ...
  const leave = (type, serial) => {
    const frames = calls[calls.length-1].frames;
    check("Leave-Type", frames[frames.length-1].type, type, serial);
    check("Leave-Serial", frames[frames.length-1].serial, serial, serial);
    frames.length--;
  };
  ...
  traps.read = (identifier, value, serial) => {
    ...
    while (index--) {
      if (identifier in call.frames[index].binding) {
        return read(call.frames[index].binding, identifier, value, serial);
      }
    }
    index = call.scope.length;
    while (index--) {
      if (identifier in call.scope[index]) {
        return read(call.scope[index], identifier, value, serial);
      }
    }
    throw new Error("["+serial+"] Read failure: "+identifier+".");
  };
  ...
  traps.closure = (value, serial) => {
    let wrapper = function () {
      ...
      if (completions.length)
        throw new Error("["+serial+"] Completion stack poluted.");
      return result;
    };
    const call = calls[calls.length-1];
    const scope = [];
